...
!(function (G) {
  ...
  function collf(fn, asy_fn) {
    return function f(d, i) {
      ...
      return (mp && (mp.__mr ? thenable_mr(mp) : mp.then && _.is_fn(mp.then))
        ? asy_fn
        : fn)(data, iter, ks, mp, i, l, k);
    };
  }
  ...
  // async not supported
  _.find_k = _.find_key = _.findKey = collf(function (
    data,
    iter,
    ks,
    X,
      ...
      !(function (_notice, keys) {
        if (_notice)
          if (_.isString(keys)) emit_loop(emit_args, _notice, keys);
            ...
            each(keys, _(emit_loop, emit_args, _notice));
